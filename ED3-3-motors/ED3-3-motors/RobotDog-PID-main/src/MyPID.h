...
double kp = 10, ki = 20, kd = 0.1;
double kptwo = 20, kitwo = 20, kdtwo = 0.1;
double kpthree = 10, kithree = 20, kdthree = 0.1;
// Input, output, reference
PID MOT1_PID(&th1, &MOT1_cmd, &th1_ref, kp, ki, kd, DIRECT);
PID MOT2_PID(&th2, &MOT2_cmd, &th2_ref, kptwo , kitwo , kdtwo , DIRECT);
PID MOT3_PID(&th3, &MOT3_cmd, &th3_ref, kpthree, kithree, kdthree, DIRECT);
...
void Compute_PID()
{
  ...
  MOT2_PID.SetTunings(kptwo , kitwo , kdtwo );
  MOT2_PID.Compute();

  MOT3_PID.SetTunings(kpthree, kithree, kdthree);
  MOT3_PID.Compute();
